...
if __name__ == '__main__':
    ...
    if args.threads is not None and int(args.threads) > 1:
        ...
        # exit()
        iterate_threaded(nthreads, pth, str(args.c).replace('\'', '"'), args.t, str(args.i).lower() in ['true', 'y', 't', 'yes'])
    else:
        print('single thread')
        # exit()
        conversions = iterate(pth, str(args.c).replace('\'', '"'), args.t, str(args.i).lower() in ['true', 'y', 't', 'yes'])
