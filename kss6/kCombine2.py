    ...
    for name in os.listdir(inD.aPath()):
        ...
        if fPath.isFile():
            ...
            if out.exists():
                continue
            #libx265
            cmd = ( f'ffmpeg -y -i "{fPath}" -c:v hevc_nvenc -crf 19'
...
subclips = list()
for f in files:
    #p = ffmpeg.input(f.path())
    #p = ffmpeg.input(f.path()).filter_multi_output('split')
    #subclips.append( p[0] )
    #subclips.append( p[1] )
    #a += 1
    #subclips.append( p['a'] )
    subclips.append( f"file \'{f.aPath()}\'\n" )

f = open("list.txt", "w+")
f.write( ''.join(subclips) )
f.close()

output = root.append(f'{randomString(7)}_compilation.mp4')
command = ""
if "-copy" in sys.argv:
    command = "ffmpeg -f concat -safe 0 -i \"" + f"{root.append('list.txt')}" + "\" -c:v copy -c:a copy \"" + f'{output.path()} -- output.mp4' + "\""
else:
    command = "ffmpeg -f concat -safe 0 -i \"" + f"{root.append('list.txt')}" + "\" -c:v hevc_nvenc -c:a aac \"" + f'{output.path()} -- output.mp4' + "\""
p = subprocess.Popen(command, cwd=root.append("results_combine").aPath())
p.communicate()

root.append('list.txt').delete()


#out = mpye.concatenate_videoclips(subclips, method='compose')
#output = root.append(f'{randomString(7)}_compilation.mp4')
#out.write_videofile(output.aPath(), codec='nvenc_hevc', audio_codec='aac', audio_bitrate='48k')



#out.write_videofile(output.aPath(), preset='ultrafast', codec='libx265', audio_codec='aac', audio_bitrate='48k')
#j = ffmpeg.concat(*subclips, v=1, a=1).node
#ffmpeg.output(j['a'], j['v'], output.aPath(), shortest=None).run(capture_stdout=True) #vcodec='copy', acodec='copy'
