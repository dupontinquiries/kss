...
for f in files:
    if "-nl" in sys.argv:
        command = "ffmpeg -i \"" + f"{f.aPath()}" + "\" -c:v hevc_nvenc -c:a aac \"" + f"{root.append('results_encoded').append(f.path())}" + "\""
    else:
        command = "ffmpeg -i \"" + f"{f.aPath()}" + "\" -c:v hevc_nvenc -c:a aac \"" + f"{root.append('results_encoded').append('[encoded] ' + f.path())}" + "\""
    p = subprocess.Popen(command, cwd=root.append("results_encoded").aPath())
    p.communicate()
