...
def process_in_groups(i, mod, c_l, spread):
    ...
    ffmpeg.run(ffmpeg.output(base_a, "final\\processed_audio_from" + i + ".wav"))
    new_audio = (ffmpeg.input("final\\processed_audio_from" + i + ".wav"))
    base_v = ret['v'].filter("atadenoise").filter('crop', x=0*1080*scale_factor, y=0, w=1080*scale_factor, h=1350*scale_factor)
...
base = process_in_groups(vid_arr[0], 1.45, 800, 7)
base_v = base['v']
base_a = base['a']
for w in range(1, len(vid_arr) - 1):
    # concat = trim_silent(ffmpeg.input(vid_arr[w+1]), w)
    to_concat = process_in_groups(vid_arr[w], 1.45, 800, 7) #1.4, 15000, 5 is pretty good with about 50% retention and near full comprehensibility
