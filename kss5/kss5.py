...
class kss:
    ...
    def writeVideo_multithreading(self, tmpChunks, referenceVideos, vidList):
        ...
        if individual_videos:
            outputMovie.write_videofile(outD.append(f'{returnChunksList[0].path()[:-4]} {randomString(3)}-out.mp4').aPath(), preset='slow', codec='libx265', audio_codec='aac', audio_bitrate='48k') #, threads=16)
    ...
    def writeVideo_v3(self, tmpChunks, vidList):
        ...
        for a, b in filters.items():
            if COPY_CODEC:
                ...
                for i in range(0, len(b)):
                    ...
                    count += 1
                idea = """for i in range(0, len(b), step):
                    end = min(len(b), i + step)
                    #tmpName = outD.append(f"{folderName}_threading").append("{:0>10d}".format(count) + '.mp4').aPath()
                    f = ffmpeg.input(a)
                    #g = list()
                    #sections.append( f.trim(start_frame=b[i][0], end_frame=b[i][1]) )
                    sections.append( f.trim(start_frame=round(b[i][0]*_fps), end_frame=round(b[i][1]*_fps)) ) #f.subclip(b[i][0], b[i][1]))
                    #ffmpeg_extract_subclip(a, b[i][0], b[i][1], targetname=tmpName)
                    #stamps = ' '.join(b[i:end])
                    #command = f"ffmpeg -i \"{a}\" -c copy {stamps} \"{tmpName}\""
        ...
        if individual_videos:
            print(f"\n\n --- video source = {vidList[tmpChunks[0][1]]}\n\n")
            #exit()
            command = "ffmpeg -f concat -safe 0 -i \"" + "list.txt" + "\" -c copy \"" + "../" + f'{vidList[tmpChunks[0][1]].path()[:-4]}.mp4' + "\"" #randomString(5)
    ...
    def __init__(self, sessID, inD, workD, outD, vidList=None):
        self.sessID = sessID
        self.inD = inD
        self.workD = workD
        if vidList == None:
